...
class Passthrough(Operations):
    ...
    def _full_path(self, partial):
        if(partial==self.root) : return partial
    ...
    def open(self, path, flags):
        full_path = self._full_path(path)
        if(not self.Auth and (not full_path.endswith(".enc"))) : return os.open(full_path+".enc", flags)
...
def lock(path) :
    ...
    for file in os.listdir(path) :
        if os.path.isfile(os.path.join(path, file)) and (not file.endswith(".enc")) : enc_file(os.path.join(path, file))
        print(file)
...
def main(mountpoint, root):
    print(f"root {root} mountpoint : {mountpoint}")
    lock(os.path.abspath(root))
